...
export class MatchService {
  ...
  addPoint(player: string) {
    ...
    if (this.currentMatch.challengerScore >= 11 && this.currentMatch.challengerScore - this.currentMatch.opponentScore == 2) {
      this.makeWinner(this.currentMatch.challenger);
    }
    else if (this.currentMatch.opponentScore >= 11 && this.currentMatch.opponentScore - this.currentMatch.challengerScore == 2) {
      this.makeWinner(this.currentMatch.opponent);
    }
    if (this.currentMatch.challengerScore + this.currentMatch.opponentScore > 20) {
      this.switchServe();
    }
